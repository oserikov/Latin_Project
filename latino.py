...
# data = ["alacer, alacris, alacre    быстрый",
#         "silence, i m    покой",
#         "iucundus, a, um    удобный",
#         "liber, libera, liberum    свободный",
#         "ydicer, ydicris, ydicre    ветренный",
#         "mortâlis, mortâlis, mortâle    смертный",
#         "audax, gen: audactis    будущий",
#         "jusiur, gen: justil    выдуманный",
#         "alea, aleae f    жребий",
#         "sapiens, gen: sapientis    мудрый",
#         "hic, haec, hoc    этот, эта, это",
#         "inferis, infere    нижний",
#         "levis, leve    широкий",
#         "emergo, mersi, mersum 3    возникаться",
#         "desum, fui, --, esse    не хватать",
#         "is, ea, id    этот, эта, это",
#         "sub + Acc. et Abl.    под",
#         "ultra+Acc    за, по ту сторону",
#         "dives, gen: divitis    богатый"]
...
def fill_3_declin_for_1end(some_arr):  #дописывает формы у 3 скл, у прил. одного окончания
    ...
    for f_arr in some_arr:
        translation = f_arr[1]
        form = f_arr[0]
        if len(form) == 2 and form[1].startswith("gen: "):
            gen_f = (form[1].replace("gen: ", ""))
            arr_gen.append([gen_f, translation])
            form[1] = form[0]
            form.append(form[0])
            third_dec_1end.append([form, translation])
...
def making_forms_3_1(some_arr, gen_arr):   #делает фомрмы прил. 3скл. одного оконч.
    ...
    caus_neut_Pl = [[[x, y, z, q, f], tn] for x, y, z, q, f, tn in zip('-', '-', '-', '-', Abls, Pl_trans)]
...
#print(res29,'\n',  res30, '\n', res31, '\n', res32, '\n', res33)
#       'Прил. 3скл. 3 оконч. ж.р. ед.ч.:',res30, '\n',
...
def craft_diction_forms(dec1_Sg, dec1_Pl, dec2_m_Sg,dec2_m_Pl, dec2_n_Sg, dec2_n_Pl, dec3_1end_mf_Sg, dec3_1end_n_Sg,
                        ...
                        dec3_2ends_n_Pl, dec3_3ends_m_Sg, dec3_ends_f_Sg, dec3_3ends_n_Sg, dec3_3ends_mf_Pl, dec3_3ends_n_Pl):

    arr_12dec = []
    forms_of_word = []
    arr_12dec.extend([dec1_Sg, dec1_Pl, dec2_m_Sg, dec2_m_Pl, dec2_n_Sg, dec2_n_Pl])
    for dic_line in arr_12dec:
        form = (dic_line[0][0])
        forms_of_word.append(form)
    #print(forms_of_word)
    m_as_keys = []
    for m_forms in dec2_m_Sg:
        m_form = (m_forms[0][0])
        m_as_keys.append(m_form)
    dict_12dec = dict((key, value) for (key, value) in zip(m_as_keys, forms_of_word))

    arr_3dec1 = []
    arr_3dec1.extend([dec3_1end_mf_Sg, dec3_1end_mf_Pl, dec3_1end_n_Sg, dec3_1end_n_Pl])

    arr_3dec2 = []
    arr_3dec2.extend([dec3_2ends_mf_Sg, dec3_2ends_mf_Pl, dec3_2ends_n_Sg, dec3_2ends_n_Pl])

    arr_3dec3 = []
    arr_3dec3.extend([dec3_3ends_m_Sg,dec3_ends_f_Sg,dec3_3ends_mf_Pl, dec3_3ends_n_Sg, dec3_3ends_n_Pl])

    return dict_12dec

res34 = craft_diction_forms(res10, res11, res12, res13, res14, res15, res16, res21, res22, res23, res25, res26, res27,
                            res28, res29, res30, res31, res32, res33)
#print(res34)