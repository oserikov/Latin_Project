...
def making_forms_1(some_arr):
    ...
    for f_arr in some_arr:
        ...
        forms = f_arr[0]
        translation = f_arr[1]
        for variant in morph.parse(translation): #смотрим варианты разбора, которые предлагает пайморфи
            if variant.tag.POS == 'ADJF': #должно быть прилагательное, а то иногда пайморфи слова в существительные записывает
                fem_trans = variant.inflect({'femn'}).word
                break
        f = (forms[1][:-1])
        pattern_er = (forms[0])
    ...
    for f_arr in some_arr:
        ...
        forms = f_arr[0]
        translation = f_arr[1]
        for variant in morph.parse(translation): 
            if variant.tag.POS == 'ADJF': 
                fem_trans = variant.inflect({'plur'}).word
                break
        f = (forms[1][:-1])
        pattern_er = (forms[0])
...
def making_forms_2_m(some_arr):
    ...
    for f_arr in some_arr:
        ...
        for variant in morph.parse(translation): 
            if variant.tag.POS == 'ADJF': 
                musc_trans = variant.inflect({'plur'}).word
                break
        f = (forms[1][:-1])
        pattern_er = (forms[0])
...
def making_forms_2_n(some_arr):
    ...
    for f_arr in some_arr:
        ...
        for variant in morph.parse(translation): 
            if variant.tag.POS == 'ADJF': 
                neut_trans = variant.inflect({'neut'}).word
                break
        f = (forms[1][:-1])
        pattern_er = (forms[0])
    ...
    for f_arr in some_arr:
        ...
        for variant in morph.parse(translation): 
            if variant.tag.POS == 'ADJF': 
                neut_trans = variant.inflect({'plur'}).word
                break
        f = (forms[1][:-1])
        pattern_er = (forms[0])
...
def making_3_dec_2_ends(some_arr):
    ...
    for f_arr in some_arr:
        ...
        trans = f_arr[1]
        TransM.append(trans)
        for variant in morph.parse(trans): 
            if variant.tag.POS == 'ADJF': 
                trans_f = variant.inflect({'femn'}).word
                TransF.append(trans_f)
                trans_n = variant.inflect({'neut'}).word
                TransN.append(trans_n)
                pl_tran = variant.inflect({'plur'}).word
                Pl_Trans.append(pl_tran)
                break
...
def making_3_dec_3_ends(some_arr):
    ...
    for f_arr in some_arr:
        ...
        for variant in morph.parse(trans): 
            if variant.tag.POS == 'ADJF': 
                trans_f = variant.inflect({'femn'}).word
                F_trans.append(trans_f)
                trans_n = variant.inflect({'neut'}).word
                N_trans.append(trans_n)
                steam = (f_arr[0][1][:-2])
                steams.append(steam)
                pl_tran = variant.inflect({'plur'}).word
                Pl_Trans.append(pl_tran)
